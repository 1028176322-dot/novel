...
校验项（FAIL=不一致须人工修；WARN=提示性）：
  ...
  G5 机械校验引用一致性：SKILL.md 与 AI写作自动化流程.md 均含 C1–C17 口径
...
def main(repo):
    ...
    for label, p in (("SKILL.md", skill_path), ("AI写作自动化流程.md", flow_path)):
        if not os.path.isfile(p):
            ...
            continue
        txt = _read(p)
        has_c17 = ("C1–C17" in txt) or ("C1-C17" in txt)
        # 反向：不应残留旧的 C1–C7 主导口径（允许出现但应同时有 C1–C17）
        if has_c17:
            oks.append("G5 机械校验引用：%s 含 C1–C17 口径" % label)
        else:
            fails.append(("G5 机械校验引用", "%s 未含 C1–C17 口径（疑似残留旧版）" % label))
